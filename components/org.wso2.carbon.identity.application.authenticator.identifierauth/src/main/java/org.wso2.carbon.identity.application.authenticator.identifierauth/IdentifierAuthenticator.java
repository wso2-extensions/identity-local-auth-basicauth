...
import org.wso2.carbon.identity.application.authentication.framework.LocalApplicationExecutor;
...
import org.wso2.carbon.identity.application.authenticator.identifierauth.internal
...
/**
 * Identifier based Authenticator
 */
public class IdentifierAuthenticator extends AbstractApplicationAuthenticator
        implements LocalApplicationExecutor {

    private static final long serialVersionUID = 1819664539416029785L;
    private static final Log log = LogFactory.getLog(IdentifierAuthenticator.class);
    private static String RE_CAPTCHA_USER_DOMAIN = "user-domain-recaptcha";

    @Override
    public boolean canHandle(HttpServletRequest request) {
        String userName = request.getParameter(IdentifierAuthenticatorConstants.USER_NAME);
        ...
        try {
            ...
            if (errorContext != null && errorContext.getErrorCode() != null) {
                ...
                if (errorCode.equals(IdentityCoreConstants.USER_ACCOUNT_NOT_CONFIRMED_ERROR_CODE)) {
                    ...
                    String redirectURL = loginPage + ("?" + queryParams) + IdentifierAuthenticatorConstants.FAILED_USERNAME
                            + URLEncoder.encode(username, IdentifierAuthenticatorConstants.UTF_8) +
                            IdentifierAuthenticatorConstants.ERROR_CODE + errorCode + IdentifierAuthenticatorConstants
                            .AUTHENTICATORS + getName() + ":" + IdentifierAuthenticatorConstants.LOCAL + retryParam;
                    ...
                    if (log.isDebugEnabled()) {
                        ...
                        log.debug("username : " + request.getParameter(IdentifierAuthenticatorConstants.USER_NAME));
                        log.debug("remainingAttempts : " + remainingAttempts);
                    }

                    if (errorCode.equals(UserCoreConstants.ErrorCode.INVALID_CREDENTIAL)) {
                        retryParam = retryParam + IdentifierAuthenticatorConstants.ERROR_CODE + errorCode
                                + IdentifierAuthenticatorConstants.FAILED_USERNAME + URLEncoder
                                .encode(request.getParameter(IdentifierAuthenticatorConstants.USER_NAME),
                                        IdentifierAuthenticatorConstants.UTF_8)
                                + "&remainingAttempts=" + remainingAttempts;
                        response.sendRedirect(loginPage + ("?" + queryParams)
                                + IdentifierAuthenticatorConstants.AUTHENTICATORS + getName() + ":" +
                                IdentifierAuthenticatorConstants.LOCAL + retryParam);
                        ...
                        if (remainingAttempts == 0) {
                            if (StringUtils.isBlank(reason)) {
                                redirectURL = response.encodeRedirectURL(redirectURL + ("?" + queryParams)) +
                                        IdentifierAuthenticatorConstants.ERROR_CODE + errorCode + IdentifierAuthenticatorConstants.FAILED_USERNAME +
                                        URLEncoder.encode(request.getParameter(IdentifierAuthenticatorConstants.USER_NAME), IdentifierAuthenticatorConstants.UTF_8) +
                                        "&remainingAttempts=0";
                            } else {
                                redirectURL = response.encodeRedirectURL(redirectURL + ("?" + queryParams)) +
                                        IdentifierAuthenticatorConstants.ERROR_CODE + errorCode + "&lockedReason="
                                        + reason + IdentifierAuthenticatorConstants.FAILED_USERNAME +
                                        URLEncoder.encode(request.getParameter(IdentifierAuthenticatorConstants.USER_NAME),
                                                IdentifierAuthenticatorConstants.UTF_8) + "&remainingAttempts=0";
                            }
                        } else {
                            if (StringUtils.isBlank(reason)) {
                                redirectURL = response.encodeRedirectURL(redirectURL + ("?" + queryParams)) +
            ...
            throw new AuthenticationFailedException(e.getMessage(), User.getUserFromUserName(request.getParameter
                    (IdentifierAuthenticatorConstants.USER_NAME)), e);
        }
    }
    ...
    protected void processAuthenticationResponse(HttpServletRequest request,
        ...
        try {
            ...
            UserRealm userRealm = IdentifierAuthenticatorServiceComponent.getRealmService().getTenantUserRealm(tenantId);
            if (userRealm != null) {
    ...
    public String getFriendlyName() {
        return IdentifierAuthenticatorConstants.AUTHENTICATOR_FRIENDLY_NAME;
    }

    @Override
    public String getName() {
        return IdentifierAuthenticatorConstants.AUTHENTICATOR_NAME;
    }
}
